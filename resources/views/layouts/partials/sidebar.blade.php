...
<aside class="main-sidebar">
    ...
    <section class="sidebar">
                        ...
                        @if (auth()->user()->can('supplier.create') || auth()->user()->can('customer.create'))
                            <li
                                ...
                                <a href="{{ action('ContactController@getImportContacts') }}"><i
                                    ...
                                    @lang('lang_v1.import_contacts')</a>
                            </li>
                        @endcan
        ...
        @if (auth()->user()->can('product.view') ||
            ...
            <li class="treeview {{ in_array($request->segment(1), ['variation-quantity', 'variation-templates', 'products', 'labels', 'import-products', 'import-opening-stock', 'selling-price-group', 'brands', 'simpan-barang', 'units', 'categories']) ? 'active active-sub' : '' }}"
                id="tour_step5">
                ...
                <ul class="treeview-menu">
                    @can('product.view')
                        <li
                            class="{{ ($request->segment(1) == 'products' && $request->segment(2) == '') || $request->segment(1) == 'variation-quantity' ? 'active' : '' }}">
          ...
          @endcan -->
                    ...
                    @can('product.opening_stock')
                        <li class="{{ $request->segment(1) == 'import-opening-stock' ? 'active' : '' }}"><a
                                href="{{ action('ImportOpeningStockController@index') }}"><i
                                    class="fa fa-download"></i><span>@lang('lang_v1.import_opening_stock')</span></a></li>
                    @endcan
                    @can('product.create')
                        <li class="{{ $request->segment(1) == 'selling-price-group' ? 'active' : '' }}"><a
                                href="{{ action('SellingPriceGroupController@index') }}"><i
                                    class="fa fa-circle-o"></i><span>@lang('lang_v1.selling_price_group')</span></a></li>
                    @endcan
                    ...
                    @endif
                    @if (auth()->user()->can('brand.view') || auth()->user()->can('brand.create'))
                        <li class="{{ $request->segment(1) == 'simpan-barang' ? 'active' : '' }}">
                            <a href="{{ action('SimpanBarangController@index') }}"><i class="fa fa-reorder"></i>
                                <span>Penyimpanan
                                    Barang</span></a>
                        </li>
                    @endif
        ...
        @if (auth()->user()->can('purchase.view') ||
            ...
            <li class="treeview {{ in_array($request->segment(1), ['purchases', 'purchase-return']) ? 'active active-sub' : '' }}"
                ...
                <ul class="treeview-menu">
                    ...
                    @endcan
                    <!--  <li class="{{ $request->segment(1) == 'pembayaran_hutang' ? 'active' : '' }}">
          <a href="{{ action('PembayaranHutangController@index') }}">
            <i class="fa fa-book"></i> <span>Pembayaran Hutang</span>
          </a>
        </li> -->
                </ul>
            </li>
        ...
        @if (auth()->user()->can('sell.view') || auth()->user()->can('sell.create') || auth()->user()->can('direct_sell.access'))
            <li class="treeview {{ in_array($request->segment(1), ['sells', 'pos', 'sell-return', 'preorder']) ? 'active active-sub' : '' }}"
                id="tour_step7">
                ...
                <ul class="treeview-menu">
                    @can('sell.sell_list')
                        <li
                            ...
                            <a href="{{ action('SellController@index') }}"><i
                                    class="fa fa-list"></i>@lang('lang_v1.all_sales')</a></li>
                        <li
                            class="{{ $request->segment(1) == 'preorder' && $request->segment(2) == null ? 'active' : '' }}">
                            <a href="{{ action('PreOrderController@index') }}"><i class="fa fa-list"></i>Daftar
                                Pre-Order</a>
                        </li>
                    @endcan
                    @can('direct_sell.access')
                        ...
                        <li
                            class="{{ $request->segment(1) == 'preorder' && $request->segment(2) == 'create' ? 'active' : '' }}">
                            <a href="{{ action('PreOrderController@create') }}"><i
                    ...
                    @can('sell.view')
                        <li
                            ...
                            <a href="{{ action('SellPosController@index') }}"><i
                                    class="fa fa-list"></i>@lang('sale.list_pos')</a></li>
                    @endcan
                    @can('sell.create')
                        <li
                    ...
                    @can('sell.create')
                        <li
                            ...
                            <a href="{{ action('SellReturnController@index') }}"><i
                                    class="fa fa-undo"></i>@lang('lang_v1.list_sell_refund')</a></li>
                    @endcan
                    @can('sell.view')
                        <li class="{{ $request->segment(2) == 'laporan-sell-periode' ? 'active' : '' }}"><a
                                href="{{ action('ReportController@getSellReportPeriode') }}"><i
                                    class="fa fa-list"></i>Daftar
                                Penjualan Periode</a></li>
                    @endcan
                    <!--  <li class="{{ $request->segment(1) == 'pembayaran_piutang' ? 'active' : '' }}">
             <a href="{{ action('PembayaranPiutangController@index') }}">
              <i class="fa fa-book"></i> <span>Pembayaran Piutang</span>
            </a>
          </li> -->
                </ul>
            </li>
        ...
        @if (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create'))
            <li class="treeview {{ $request->segment(1) == 'stock-adjustments' ? 'active active-sub' : '' }}">
                ...
                <ul class="treeview-menu">
                    @can('purchase.view')
                        <li
                            class="{{ $request->segment(1) == 'penyesuaian' && $request->segment(2) == 'daftar-stock-opname' ? 'active' : '' }}">
                            <a href="{{ action('StockAdjustmentController@getDaftarSO') }}"><i
                                    class="fa fa-list"></i>Daftar Stok
                                Opname</a>
                        </li>
                    @endcan
                    @can('purchase.create')
                        <li
                            ...
                            <a href="{{ action('StockAdjustmentController@getStockAdjustment') }}"><i
                                    class="fa fa-plus-circle"></i>Tambah Stok Opname</a>
                        </li>
                    @endcan
                </ul>
            </li>
        ...
        @can('stock_report.view')
            <li class="{{ $request->segment(2) == 'stock-report' ? 'active' : '' }} ">
                <a href="{{ action('ReportController@getStockReport') }}">
                    <i class="fa fa-database" aria-hidden="true"></i> <span>Stok</span>
                </a>
            </li>
        @endcan

        <!--<li class="{{ $request->segment(2) == 'stock-report' ? 'active' : '' }}">-->
        <!--  <a href="{{ url('reports/stock-report') }}">-->
    ...
    @endif -->
        ...
        @if (auth()->user()->can('purchase_n_sell_report.view') ||
            ...
            <li></li>
            <!-- <li class="{{ $request->segment(1) == 'tutup_buku' ? 'active' : '' }}">
        <a href="{{ action('TutupBukuController@index') }}">
          <i class="fa fa-book"></i> <span>Tutup Buku</span>
        </a>
      </li> -->
            <!--<li class="{{ $request->segment(1) == 'tutup-saldo' ? 'active' : '' }}">-->
            <!--  <a href="/tutup-saldo">-->
            <!--    <i class="fa fa-money"></i> <span>Tutup Saldo Bulanan</span>-->
            <!--  </a>-->
            <!--</li>-->
            <li class="{{ $request->segment(1) == 'laporan' ? 'active' : '' }}">
                <a href="{{ action('LaporanController@index') }}">
                    <i class="fa fa-money"></i> <span>Laporan</span>
                </a>
            </li>
            <!-- @can('purchase.create')
          <li class="{{ $request->segment(1) == 'stock-adjustments' && $request->segment(2) == 'create' ? 'active' : '' }}"><a href="{{ action('TutupBukuController@index') }}"><i class="fa fa-clipboard"></i>Tutup Buku</a></li>
    @endcan -->
            @can('jurnal.view')
                <li class="{{ $request->segment(1) == 'jurnal' ? 'active' : '' }}">
                    <a href="{{ action('JurnalController@index') }}">
                        <i class="fa fa-book"></i> <span>Transaksi Umum</span>
                    </a>
                </li>
            @endcan
            <li style="display: none;"
                class="treeview {{ in_array($request->segment(1), ['reports']) ? 'active active-sub' : '' }}"
                id="tour_step8">
                    ...
                    @can('purchase_n_sell_report.view')
                        <li class="{{ $request->segment(2) == 'product-purchase-report' ? 'active' : '' }}"><a
                                href="{{ action('ReportController@getproductPurchaseReport') }}"><i
                                    class="fa fa-arrow-circle-down"></i>@lang('lang_v1.product_purchase_report')</a></li>
