@extends('layouts.app')
@section('title', $returnType == 'return' ? __('lang_v1.purchase_return') : __('lang_v1.purchase_return_consignment'))
...
@section('content')
    ...
    <section class="content-header">
        <h1>
            ...
            @else
                @lang('lang_v1.purchase_return_consignment')
            @endif
        </h1>
    </section>
        ...
        <div class="box box-solid">
            <div class="box-body">
                            ...
                            <tbody>
                                @foreach ($purchase->purchase_lines as $purchase_line)
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="display_currency"
                                                data-currency_symbol="false">{{ $qty_available }}</span></td>
                                        <td>
                                            @php
                                                $check_decimal = 'false';
                                            ...
                                            <input type="text" name="returns[{{ $purchase_line->id }}]"
                                                value="{{ @num_format($purchase_line->quantity_returned) }}"
				              			...
				              			data-msg-max-value="@lang('validation.custom-messages.quantity_not_available', ['qty' => $purchase_line->formatted_qty_available, 'unit' => $purchase_line->product->unit->short_name ])" @endif>
                                            <input type="hidden" class="unit_price"
                                                value="{{ @num_format($purchase_line->purchase_price_inc_tax) }}">
                                        </td>
                                        <td>
                ...
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            {!! Form::label('Jumlah', 'Jumlah Bayar :') !!}
                            {!! Form::text('jumlah', @num_format(0), ['class' => 'jumask form-control input_number', 'id' => 'jumlah']) !!}
                        </div>
                    </div>
                    @if ($returnType == 'consignment')
                        <input type="hidden" name="method" id="method" value="konsinyasi">
                    @else
                        <div class="col-sm-4">
                            <div class="form-group">
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        function update_purchase_return_total() {
            ...
            $('input#jumlah').val(numberWithCommas(net_return_inc_tax));
