...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id)
    {
        ...
        $tbt = $invoice_total['total_before_tax'];
        $fintot = $this->num_uf($input['final_total']);
        ...
        $transaction = Transaction::create([
            ...
            'is_hutang_piutang' => isset($input['is_hutang_piutang']) ? $input['is_hutang_piutang'] : 0
        ]);
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [])
    {
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $upit = $this->num_uf($product['unit_price_inc_tax']);
                $line = [
     ...
     * @return boolean
     */
    public function accountNumbering($transaction_type, $payment_method, $tipe_bayar, $rek_type = null)
    {
        $kd_rekening_kredit  = '';
        $kd_rekening_debit   = '';
        $kd_jenis_buku       = '';
        ...
        switch ($transaction_type) {
            case 'sell':
                ...
                if ($payment_method == 'cash') {
                    if ($tipe_bayar == 'lunas') {
                        if ($rek_type != null) {
                            if ($rek_type == 'laba') {
                                $kd_rekening_kredit   = '411.01';
                                $kd_rekening_debit    = '111.04';
            ...
            case 'purchase':
                ...
                if ($payment_method == 'cash') {
                    if ($tipe_bayar == 'pembayaran_sebagian') {
            ...
            case 'sell_return':
                if ($payment_method == 'cash') {
                    if ($tipe_bayar == 'lunas') {
                        if ($rek_type != null) {
                            if ($rek_type == 'laba') {
                                $kd_rekening_kredit    = '411.05';
                                $kd_rekening_debit   = '111.44';
                            } elseif ($rek_type == 'harga_pokok_penjualan') {
                                $kd_rekening_kredit  = '111.12';
                                $kd_rekening_debit   = '131.04';
                            }
                        }
                    } elseif ($tipe_bayar == 'pembayaran_sebagian') {
                        if ($rek_type != null) {
                            if ($rek_type == 'laba') {
                                //penyaluran piutang retur laba
                                $kd_rekening_kredit = '211.05';
                                $kd_rekening_debit = '411.06';
                            } elseif ($rek_type == 'harga_pokok_penjualan') {
                                //penyaluran piutang retur hpp
                                $kd_rekening_kredit  = '211.04';
                                $kd_rekening_debit   = '131.09';
                            }
                        }
                    } elseif ($tipe_bayar == 'hutang') {
                        if ($rek_type != null) {
                    ...
                    if ($tipe_bayar == 'lunas') {
                        if ($rek_type != null) {
                            if ($rek_type == 'laba') {
                                $kd_rekening_kredit   = '121.34';
                                $kd_rekening_debit    = '411.20';
                            } elseif ($rek_type == 'harga_pokok_penjualan') {
                                $kd_rekening_kredit   = '121.33';
                                $kd_rekening_debit    = '131.16';
                            }
                        }
                    } else {
                        if ($rek_type != null) {
            ...
            case 'purchase_return':
                if ($payment_method == 'cash') {
                    if ($tipe_bayar == 'lunas') {
                        if ($rek_type = 'pr') {
                            $kd_rekening_kredit  = '131.07';
                            $kd_rekening_debit   = '111.05';
                        }
                    } elseif ($tipe_bayar == 'pembayaran_sebagian') {
                        if ($rek_type = 'pr') {
                            $kd_rekening_kredit  = '131.10';
                            $kd_rekening_debit   = '132.05';
                        }
                    } elseif ($tipe_bayar == 'hutang') {
                        if ($rek_type = 'pr') {
                            $kd_rekening_kredit  = '131.10';
                    ...
                    if ($tipe_bayar == 'lunas') {
                        if ($rek_type == 'pr') {
                            $kd_rekening_kredit     = '131.17';
                            $kd_rekening_debit      = '121.35';
                        }
                    } else {
                        if ($rek_type == 'pr') {
                            $kd_rekening_kredit     = '131.10';
        ...
        return array(
            'kd_rekening_kredit' => $kd_rekening_kredit,
            'kd_rekening_debit' => $kd_rekening_debit
        );
    }
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null)
    {
        ...
        foreach ($payments as $key) {
            if ($key['rek_type'] != 'pembayaran_biaya_kirim') {
        ...
        foreach ($payments as $payment) {
            $duit = $payment['amount'];
            $conv = $this->num_uf($duit);
            $flconv = "$conv";
            if ($transaction->type == 'purchase') {
                $sisa = $transaction->final_total - $ttl_payment;
            } else {
                $sisa = $transaction->final_total - $this->num_uf($payment['pay']);
            }

            $tipe_bayar = '';
            if ($sisa > 0) {
                if ($sisa == $transaction->final_total) {
                    $tipe_bayar = 'hutang';
                } else {
                ...
                if ($this->num_uf($payment['amount']) != 0) {
                    ...
                    if ($transaction->status == 'received' || $transaction->status == 'final') {
                        if (isset($payment['rek_type'])) {
                            ...
                            $accountNum = $this->accountNumbering($transaction->type, $payment['method'], $tipe_bayar);
                        }
                    } else {
                        $accountNum = null;
                    }
    ...
    public function getSellLines($transaction_id)
    {
        $payment_lines = TransactionSellLine::where('transaction_id', $transaction_id)
            ->get()->toArray();
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $transaction_payment = TransactionPayment::where([
            ['transaction_id', $transaction_id],
            ['id_rekening_debit', '111.04']
        ])->first();
        ...
        //Logo
        if (!empty($il->logo) && $il->show_logo != 0) {
            $output['logo'] = $il->logo;
        } elseif (empty($output['display_name'])) {
            $output['logo'] = "no_image";
        } else {
            $output['logo'] = "logo.jpeg";
        }

        // ($il->show_logo != 0 && !empty($il->logo) && Storage::exists('invoice_logos/' . $il->logo)) ? config('app.url') . Storage::url('invoice_logos/' . $il->logo) : false;

        $output['kota'] = $location_details->city;
        $output['jml_tempo'] = $transaction->pay_term_number;
        $output['tipe_tempo'] = $transaction->pay_term_type;
        $output['tgl_trans'] = \Carbon::createFromFormat('Y-m-d H:i:s', $transaction->transaction_date)->format('d M Y');
        $output['tgl_trans2'] = $transaction->transaction_date;
        //Address
        $output['address'] = '';
        $temp = [];
        ...
        $output['customer_almt1'] = $customer->landmark;
        $output['customer_almt2'] = $customer->city;
        $output['customer_almt3'] = $customer->mobile;
        ...
        if ($transaction->payment_status == 'paid') {
            $output['tipe_byr'] = "LUNAS";
        } elseif ($transaction->payment_status == 'partial') {
            $output['tipe_byr'] = "Dibayar Sebagian";
        } else {
            $output['tipe_byr'] = "Tempo";
        }
        ...
        // $output['invoice_tgl'] = \Carbon::createFromFormat('Y-m-d', $transaction->transaction_date)->toFormattedDateString();
        ...
        $output['discount_label'] .= ($transaction->discount_type == 'percentage' || $transaction->discount_type == 'fee') ? ' (' . $transaction->discount_amount . '%) :' : '';

        if ($transaction->discount_type == 'percentage' || $transaction->discount_type == 'fee') {
        ...
        $output['tax_label'] .= ':';
        $output['tax'] = ($transaction->tax_amount != 0) ? $transaction->tax_amount : 0;
        ...
        $output['table_tax_headings'] = !empty($il->table_tax_headings) ? json_decode($il->table_tax_headings) : null;

        return (object)$output;
    }
    public function getTutupKasir($invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        $il = $invoice_layout;
    ...
    protected function _receiptDetailsSellLines($lines, $il, $is_product_expiry_enabled, $is_lot_number_enabled)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'unit_price_exc_tax' => $line->unit_price,
    ...
    public function getTotalPaid($transaction_id)
    {
        ...
        if ($is_hutang_piutang == 1) {
            ...
            $gtotal_paid->whereRaw('LEFT(id_rekening_kredit,2) != 41');
    ...
    {
        $get_total_paid_shipping = TransactionPayment::selectRaw('SUM(amount) as total_paid_shipping')
            ->where('transaction_id', $transaction_id);

        if ($transaction_type == 'purchase') {

            $get_total_paid_shipping->where('id_rekening_debit', '=', '213.04');
        } else {

            $get_total_paid_shipping->where('id_rekening_debit', '=', '111.36');
        }

        $total_paid_shipping = $get_total_paid_shipping->first();

        return $total_paid_shipping;
    }
    /**
     ...
     * @return string
     */
    public function updatePaymentStatus($transaction_id, $final_amount = null, $shipping_charges = null, $type_transaction = null)
    {
        ...
        if ($status == 'due' || $status == 'partial') {
            if ($type_transaction == 'purchase') {
                ...
                $value += [
                    'is_hutang_piutang' => 1,
                    'kd_rekening_debit' => '131.03',
                    'kd_rekening_kredit' => '211.01'
                ];

                if ($shipping_charges > 0) {
                    //penerimaan hutang transport
                    $value += [
                        'kd_rekening_debit_htg_biaya_kirim' => '111.33',
                        'kd_rekening_kredit_htg_biaya_kirim' => '213.03'
                    ];
                }
            } elseif ($type_transaction == 'sell') {
                //penjualan barang tempo
                /* $value += ['is_hutang_piutang'=>1,
                            'kd_rekening_debit'=>'132.01',
                            'kd_rekening_kredit'=>'131.08'
                          ];*/
                ...
                if ($shipping_charges > 0) {
                    //piutang lain lain 1
                    $value += [
                        'kd_rekening_debit_htg_biaya_kirim' => '133.01',
                        'kd_rekening_kredit_htg_biaya_kirim' => '111.35'
                    ];
                }
            }
        }
    ...
    //untuk mengambil dan SUM qty SO by kurang / tambah.
    //SUM(IF(`kurang_tambah`='tambah', quantity, -1*quantity))
    public function getQtySo($product_id, $variation_id, $plineid)
    {
        $stock_so = StockAdjustmentLine::leftjoin('transaction_sell_lines_purchase_lines as TSP', 'stock_adjustment_lines.id', '=', 'TSP.stock_adjustment_line_id')
            ->where('stock_adjustment_lines.product_id', $product_id)
            ->where('stock_adjustment_lines.variation_id', $variation_id)
            ->where('TSP.purchase_line_id', $plineid)
            // ->select(DB::raw("SUM(IF(kurang_tambah = 'tambah', stock_adjustment_lines.quantity, -1*stock_adjustment_lines.quantity)) as qty_so"))
            ->select('TSP.purchase_line_id as id_pl', DB::raw("SUM(IF(kurang_tambah = 'tambah', stock_adjustment_lines.quantity, -1*stock_adjustment_lines.quantity)) as qty_so"))
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $product_id = null, $variation_id = null, $check_expiry = true, $product_stock = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            $rows = $query->select(
                ...
                DB::raw('((PL.quantity + PL.quantity_adjusted) - (PL.quantity_sold + PL.quantity_returned)) AS quantity_available'),
                ...
                ->get();
            // $hitung = $rows->count();
            // if($mapping_type == 'stock_adjustment' && $hitung == 0){
            //     $pen = 1;
            // dd($rows);
            // }
            ...
            $qty_input_so = $line->quantity;
            // dd($qty_selling);

            foreach ($rows as $k => $row) {
                $qty_allocated = 0;
                // $qty_so = 0;
                // $tot_so = $this->getQtySo($line->product_id, $line->variation_id);

                // dd($qty_selling);

                // dd($pline);
                // $qtyttt = $row->quantity_available + $stock_so;
                // dd($row->quantity_available);
                //jika qty adjus <= qty avail, maka boleh &
                // if()
                ...
                if ($mapping_type == 'stock_adjustment') {
                    $tot_so = $this->getQtySo($line->product_id, $line->variation_id, $row->purchase_lines_id);
                    ...
                    $purchase_adjustment_map[] =
                        [
                            ...
                            'quantity' => $qty_input_so,
                            'created_at' => \Carbon::now(),
                            'updated_at' => \Carbon::now()
                        ];
                    // dd($purchase_adjustment_map);
                    //Update purchase line
                    // $qty_so = 0;
                    // $qty_so = $tot_so->qty_so;
                    // $nul = "tidak ada";
                    // dd($nul);
                    // dd($tot_so);
                    // $kurtam = $line->kurang_tambah;
                    // dd($kurtam);
                    $qty_selling = 0;
                    $kurtam = $line->kurang_tambah;
                    if (empty($tot_so)) {
                        $qty_sop = $kurtam == "kurang" ? $qty_input_so * -1 : $qty_input_so;
                    } else {
                        // $kurtam = $line->kurang_tambah;
                        $qty_kt = $kurtam == "kurang" ? $qty_input_so * -1 : $qty_input_so;
                        $qty_sop = $tot_so != "null" ? $tot_so->qty_so + $qty_kt : $qty_kt;
                    }
                    // dd($qty_sop);

                    // dd($nul);
                    PurchaseLine::where('id', $row->purchase_lines_id)
                        ->update(['quantity_adjusted' => $qty_sop]);
                    ...
                    //     $qty_allocated = $row->quantity_available;
                    // }
                    // $cek = TransactionSellLinesPurchaseLines::where('sell_line_id',$line->id);
                    // if(empty($cek)){
                    ...
                    if ($line->product_id == $row->product_id && $line->variation_id == $row->variation_id) {
                        if ($qty_allocated > 0) {
                        ...
                        //Update purchase line
                        PurchaseLine::where('id', $row->purchase_lines_id)
                            ->update(['quantity_sold' => $row->quantity_sold + $qty_allocated, 'id_rekening_debit' => 'benar']);
                    } else {
                        PurchaseLine::where('id', $row->purchase_lines_id)
                            ->update(['id_rekening_debit' => $line->product_id, 'id_rekening_kredit' => 'salah']);
                    }
                }
            ...
            if (!($qty_selling == 0 || is_null($qty_selling))) {
                ...
                if ($mapping_type == 'purchase') {
                    $mismatch_error = trans(
                        "$mismatch_name-" . $line->variation_id,
            ...
            // $cek_arr[]= $purchase_adjustment_map;
            // dd($purchase_adjustment_map);
            //Insert the mapping
    ...
    public function mapAdjustmentZero($business, $transaction_lines, $mapping_type)
    {
        // dd($transaction_lines);
        if (empty($transaction_lines)) {
            return false;
        ...
        foreach ($transaction_lines as $line) {
            ...
            foreach ($rows as $k => $row) {
                ...
                if (empty($tot_so)) {
                    // dd($kurtam);
                    $qty_sop = $kurtam == "kurang" ? $qty_allocated * -1 : $qty_allocated;
                } else {
                    // $kurtam = $line->kurang_tambah;
                    $qty_kt = $kurtam == "kurang" ? $qty_allocated * -1 : $qty_allocated;
                    $qty_sop = $tot_so->qty_so + $qty_kt;
                }
                // dd($qty_sop);
                //Check for sell mapping or stock adjsutment mapping
                if ($mapping_type == 'stock_adjustment') {
    ...
    public function mapPurchaseQuantityForDeleteStockAdjustment($line_ids)
    {
        ...
        $adjust_del = StockAdjustmentLine::leftjoin('transaction_sell_lines_purchase_lines as TSP', 'stock_adjustment_lines.id', '=', 'TSP.stock_adjustment_line_id')
            ->whereIn('stock_adjustment_lines.id', $line_ids)
            ->select('TSP.purchase_line_id as id_pl', 'stock_adjustment_lines.kurang_tambah as kurtam', 'stock_adjustment_lines.quantity as qty')
            // ->orderBy('id', 'desc')
            ->get();

        foreach ($adjust_del as $row) {
            $pur_line = PurchaseLine::where('id', $row->id_pl);

            if ($row->kurtam == 'kurang') {
                $pur_line->increment('quantity_adjusted', $row->qty);
            } else {
                $pur_line->decrement('quantity_adjusted', $row->qty);
            }
            // dd($row->kurtam);
        }
        ...
        TransactionSellLinesPurchaseLines::whereIn('stock_adjustment_line_id', $line_ids)
            ->delete();
        //delete stock adjustment line
        StockAdjustmentLine::whereIn('id', $line_ids)->delete();
    ...
    public function getSupplierPurchase($business_id, $start, $end)
    {
        $query = Transaction::join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
            ->leftjoin('products as p', 'pl.product_id', '=', 'p.id')
        ...
        if (!empty($start) && !empty($end)) {
            $query->whereBetween(DB::raw('date(transactions.transaction_date)'), [$start, $end]);
        }
