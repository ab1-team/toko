...
class PurchaseReturnController extends Controller
{
    ...
    public function add($id)
    {
        ...
        return view('purchase_return.add')
            ->with(compact('purchase', 'payment_types', 'returnType'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($purchase->purchase_lines as $purchase_line) {
                ...
                if ($return_quantities[$purchase_line->id] > 0) {
                    // $purchase_line->id_rekening_debit  = '111.05';
                    // $purchase_line->id_rekening_kredit = '131.07';
                    $purchase_line->created_at         = date('Y-m-d H:i:s');
                }
                $purchase_line->save();
            ...
            $sisa = $return_total_inc_tax - $jumlah;
            $tipe_bayar = '';
            if ($sisa > 0) {
                if ($sisa == $return_total_inc_tax) {
                    $tipe_bayar = 'hutang';
                } else {
            ...
            $method = $input['method'];
            $rekening = $this->transactionUtil->accountNumbering('purchase_return', $method, $tipe_bayar, 'pr');

            if ($method == 'konsinyasi') {
                $method = 'cash';
            }

            $bayar_retur = [];
            $bayar_retur['amount']                = $jumlah;
            $bayar_retur['paid_on']               = \Carbon::now()->toDateTimeString();
            $bayar_retur['created_by']            = auth()->user()->id;
            $bayar_retur['transaction_id']        = $return_transaction->id;
            $bayar_retur['payment_for']           = $purchase->contact_id;
            $bayar_retur['method']                = $method;
            $bayar_retur['card_type']             = 'credit';

            $hitung = $this->transactionUtil->setAndGetReferenceCount('purchase_return');
            //Generate reference number
            $bayar_retur['payment_ref_no'] = $this->transactionUtil->generateReferenceNumber('purchase_return', $hitung);
            $bayar_retur['business_id'] = $request->session()->get('business.id');
            $bayar_retur['id_rekening_debit']  = $rekening['kd_rekening_debit'];
            $bayar_retur['id_rekening_kredit'] = $rekening['kd_rekening_kredit'];
            // dd($bayar_retur);
            TransactionPayment::create($bayar_retur);
